...
export function StatusBadge({
  ...
  return (
    <span
      className={`inline-flex whitespace-nowrap rounded-full border px-3 py-1 text-[10px] font-black uppercase tracking-wider ${className}`}
    >
      {status}
    </span>
  );
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="bg-slate-50 text-[10px] uppercase tracking-wider text-slate-400 dark:bg-slate-950">
            {columns.map((c) => (
              <th key={c} className="px-4 py-3 text-left font-black">
                {c}
              </th>
            ))}
          </tr>
